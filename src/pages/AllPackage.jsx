...
//   try {
//     await axiosAuth.delete(`${BASE_URL}/api/packages/${id}`);
...
function AllPackage() {
  ...
  // 🔐 axios instance WITH token (FIXED)
  const axiosAuth = axios.create({
    baseURL: BASE_URL,
  });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axiosAuth.delete(`/api/packages/${id}`);
      alert("🗑️ Package deleted!");
      fetchPackages();
    } catch (err) {
      console.error(
  ...
  const handleUpdate = async (updatedData) => {
    try {
      ...
      await axiosAuth.put(`/api/packages/${id}`, updatedData);
